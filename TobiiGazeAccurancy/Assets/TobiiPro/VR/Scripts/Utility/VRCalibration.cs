            ...
            if (Input.GetKeyDown(_startKey))
            {
                var calibrationStartResult = StartCalibration(
                    resultCallback: (calibrationResult) =>
                        GameObject.FindObjectOfType<FileLogger>().printProgress("CalibrationResult\t" + (calibrationResult ? "successful" : "unsuccessful"))
                    );

                GameObject.FindObjectOfType<FileLogger>().printProgress("CalibrationStart\t" + (calibrationStartResult ? "" : "not ") + "started");
            }
        }
    }
}
